...
router.get('/clients', async (req, res) => {
  ...
  const [rows] = await pool.query(
    'SELECT client_id,name,email,created_at FROM client'
  );

  await redisClient.setEx(key, 60, JSON.stringify(rows)); // Cache
  console.timeEnd(`Fetch:${key}`);
...
// Protected: full overview of everything
router.get('/full-overview', auth, async (req, res) => {
  ...
  const [data] = await pool.query(
    `SELECT c.client_id, c.name AS client,
            m.mic_id, m.name AS service,
            e.env_id, e.env_type,
            se.service_env_id, se.base_url, se.target_url,
            se.port, se.is_active
     FROM client c
